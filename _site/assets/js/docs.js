;(function() {
    ...
    function getReferSearchValue(url) {
        ...
        return (
            _.findKey(pattern && _.prototype, function(value, key) {
    ...
    var Menu = React.createClass({
            ...
            this.setState({
                content: this.state.content.map(function(collection) {
                    ...
                    var functions = collection.functions.map(function(entry) {
                        var entryVis =
                            found || searcher.isMatch(entry.name) || searcher.isMatch(entry.href.split('#')[1])
                    ...
                    return changed ? _.assign({}, collection, { functions: functions, visible: visible }) : collection
                }),
        ...
        onClickMenuItem: function() {
            if (++clickCount % 4 === 0 && typeof _carbonads != 'undefined' && typeof _carbonads.refresh == 'function') {
                try {
                    _carbonads.refresh()
                } catch (e) {}
            }
        ...
        onDocumentKeyDown: function(event) {
            var key = event.key || event.keyIdentifier
            if ((key == 'Tab' || key == 'U+0009') && toc.classList.contains('open') && event.target === focusLast) {
                // Restart tab cycle.
                event.preventDefault()
                focusFirst.focus()
            }
                        ...
                        collection.functions.map(function(entry, subIndex, entries) {
                            ...
                            return React.createElement(
                                ...
                                React.createElement(
                                    ...
                                    React.createElement.apply(React, ['code', null].concat(children))
                                )
                            )
                        })
                    )
    ...
    ReactDOM.render(React.createElement(Menu), toc)
